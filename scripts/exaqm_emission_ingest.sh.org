...
if [ ${USER::4} != ops. ]; then
  ...
  export maillist="simon.hsiao@noaa.gov"
fi
sendmail=${sendmail:-Y}
...
#################################
 if [ $sendmail = 'Y' ]; then
   ...
   echo "The monthly ARL AQM emission files have been successfully copied " > email_body
   echo "from $in_dir to $out_dir. " >> email_body
   echo "Please double check and validate." >> email_body
   echo TMPDIR=$TMPDIR
   [ -n "$TMPDIR" ] && ls -ld $TMPDIR
   cat email_body |mail.py -s "$subject" $maillist -v
 fi
fi
